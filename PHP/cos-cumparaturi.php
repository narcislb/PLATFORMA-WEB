...
if ($products_in_cart) {
    ...
    $stmt = $db->prepare('SELECT p.*, i.nume_imagine, v.nume_firma FROM tbl_produse p 
LEFT JOIN tbl_imagini i ON p.id = i.id_produs 
        ...
        <table>
            ...
            <tbody>
                ...
                <?php  foreach ($groupedProducts as $key => $products): ?>
                    ...
                    <?php list($vanzatorId, $vanzatorName) = explode('-', $key); ?>
    
    <!-- Display the seller's name -->
    <tr><td colspan='5' class='seller-name-header'>Nume firma:<?= htmlspecialchars($vanzatorName) ?></td></tr>
